...
class Writer:
    def __init__(self, template_invoice_path, template_etq_path, list_file, export_path, logo_path):
    ...
    def set_filename(self, row, type):
        filename = Path(
            self.export_path,
            type,
            f"{row['#']}_{row['DATE ACTIVITÉ'].date()}_{row['ORGANISME'][:10]}.xlsx"
        )
        return f'{filename}'
    ...
    def write_invoice(self, row, filename):
        ...
        worksheet['B5'] = row['DATE ACTIVITÉ'].date()

        worksheet['B9'] = row['ORGANISME']
        worksheet['B10'] = row['RESPONSABLE']
        worksheet['B11'] = row['CELLULAIRE']
        worksheet['B12'] = row['TÉL BUREAU']
        worksheet['B13'] = row['TÉL RÉSIDENCE']
        ...
        worksheet['F17'] = row['CANNES']
        worksheet['F24'] = row['GUIDES']
        worksheet['F25'] = row['BANNIÈRES']
    ...
    def write_etq(self, row, filename):
        ...
        info = f"#{row['#']} - {row['ORGANISME']}_n_" \
               f"{row['DATE ACTIVITÉ'].date()}"

        coord = f"{row['RESPONSABLE']}_n_" \
                f"Cellulaire : {row['CELLULAIRE']}_n_" \
                f"bureau : {row['TÉL BUREAU']}_n_" \
                f"Maison : {row['TÉL RÉSIDENCE']}"

        address = f"{row['address_1']}_n_" \
                  f"{row['address_2']}_n_" \
                  f"{row['address_3']}"
